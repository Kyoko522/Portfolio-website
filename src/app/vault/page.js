...
async function tryDecrypt(password) {
  ...
  const ciphertextTag = bytes.slice(28) // ciphertext || authTag (Web Crypto layout)
...
export default function VaultPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (plaintext) {
      // Store only the decrypted payload — nothing readable ships in the bundle
    ...
    <main className="gate">
      <div className={`card${shake ? ' shake' : ''}`}>
        ...
        <form onSubmit={handleSubmit} className="form">
          ...
          />
          {error && <p className="errmsg">{"// access denied — invalid passphrase"}</p>}
      ...
      <style>{`
        ...
        .inp:focus {
          ...
          box-shadow: 0 0 18px rgba(132,94,194,0.2);
        }
        .inp.err {
          border-color: #ff6b6b;
